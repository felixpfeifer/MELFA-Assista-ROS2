#include "melfa_assista_hardware/gpio_controller.hpp"
...
namespace melfa_assista_hardware {
    ...
    controller_interface::return_type GPIOController::update(
        ...
        for (size_t i = 0; i < state_interfaces_.size(); i++) {
            RCLCPP_DEBUG(
                    logger_, "%s: (%f)", state_interfaces_[i].get_name().c_str(),
        ...
        for (size_t i = 0; i < command_interfaces_.size(); i++) {
            ...
            RCLCPP_DEBUG(
                    logger_, "%s: (%f)", command_interfaces_[i].get_name().c_str(),
...
PLUGINLIB_EXPORT_CLASS(
        melfa_assista_hardware::GPIOController, controller_interface::ControllerInterface)